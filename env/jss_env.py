...
class JssEnv(gym.Env):
    ...
    def __init__(self, env_config: Optional[Union[Dict[str, Any], str, Path]] = None):
        """
        ...
        """
        # 改为支持传入纯路径字符串/Path，或传统的 env_config 字典
        if env_config is None:
            instance_path = Path(__file__).parent.absolute() / "instances" / "ta80"
        elif isinstance(env_config, dict):
            instance_path = env_config.get("instance_path", Path(__file__).parent.absolute() / "instances" / "ta80")
        else:
            instance_path = Path(env_config)
    ...
    def reset(self, seed: Optional[int] = None, options: Optional[Dict[str, Any]] = None) -> Tuple[Dict[str, np.ndarray], Dict[str, Any]]:
        ...
        Returns:
            Initial observation as a dictionary with keys:
                ...
                - 'action_mask': Boolean mask of legal actions
        """
        # 调用基类的 reset 来初始化随机数生成器并处理 seed
